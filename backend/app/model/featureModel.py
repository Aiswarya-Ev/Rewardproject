...
def checkBadge(student_id,assessment_id,attendance_score):
    ...
    if existing_record:
        ...
        if int(attendance_score) >= 8:
            ...
            db.commit()
            addBadgeacoin(student_id,assessment_id)
            return {'message': 'you got badge and super coins'}
        else:
            ...
            db.commit()
            return {'message': 'Not eligible for badge, updated assaessment score'}
...
def addcertifatecoin(student_id,course_id):
    ...
    cursor.execute('update tb_supercoin set coins=%s where student_id=%s',(coins,student_id))
    db.commit()
    return {'status': 'success', 'message': 'coins added'}
...
def certificate(student_id,assessment_id):
    ...
    if int(percentage)>60:
        ...
        db.commit()
        addcertifatecoin(student_id,course_id)
        return {'status': 'success', 'message': 'Eligible for certificate and you got super coins'} 
    else:
        status="completed"
        update_certificate = 'UPDATE tb_enrollment SET score=%s,status=%s WHERE student_id = %s AND course_id = %s' 
        cursor.execute(update_certificate,(total_score,status,student_id, course_id))                      
        db.commit()
        return {'status': 'success', 'message': 'Not eligible for certificate'}
def certification(student_id,assessment_id,score):
    ...
    if total_assessment==completed_assessment:
        ...
        return {
            'status': 'success',
            'badge_message': badge_result.get('message', ''),
            'cert_message': cert_result.get('message', '')
        }
